...
void flightmenu(void)
{
	...
	while (ch != 'g' && ch != 'G')
	{
		...
		puts("b) Show list of emopty seats.");
...
void showalphabetical(void)
{
	...
	printf("Here is a list of seat assignment for flight number %d:\n", flight->id);
	...
	for (int i = 0; i < CAPACITY; i++)
	{
		if (flight->seats[i].assigned)
			printf("%-*s %-*s %-12d %-12s\n", MAXNAML, flight->seats[i].last,
					MAXNAML, flight->seats[i].first, flight->seats[i].id, flight->seats[i].confirmed ? "Confirmed" : "Unconfirmed");
	}
}
...
void assigncustomer(void)
{
	...
	if (id < 0 && id >= CAPACITY)
	{
		puts("This seat number is invaid, Aborting.");
		return;
	}
