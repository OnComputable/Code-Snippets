...
void showalphabetical(void)
{
	printf("%-*s %-*s %-12s\n", MAXNAML, "Last Name", MAXNAML, "First Name", "Seat Number");
	for (int i = 0; i < CAPACITY; i++)
	{
		if (seats[i].assigned)
...
void deleteassignment(void)
{
	...
	puts("Would you like to delete this assignment? Enter [n/N] to abort " "or any other key to continue.");
	if ((ch = getchar()) != '\n')
		while (getchar() != '\n') continue;
	if (ch == 'n' || ch == 'N')
	{
